  ...
  <div class="flex flex-col max-w-6xl mx-auto container items-center py-8">
    ...
    >
      <div
          ...
          :key="repo.id"
      >
        <GithubRepoCard owner="PCL-Community" :repo="repo.name"/>
      </div>
    </div>
  </div>
</template>
...
async function getOrgRepoList(name: string) {
  try {
    return (await axios.get(`https://cdn.akaere.online/api.github.com/orgs/${name}/repos`)).data;
  } catch (error) {
    console.error(error);
    return null;
  }
